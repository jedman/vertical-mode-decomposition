module quicksort_mod
   ...
   contains
      ...
      subroutine quicksort(x,index)
         ...
         real, dimension(size(x)) :: x2
         integer, dimension(:), allocatable :: index2
         integer :: i

         if (present(index)) then

            if (size(index) .ne. size(x)) then
               print *,'Error in quicksort: size(index) does not equal size(x)'
               stop
            end if

            call sortrx(size(x),x,index)
            ...
            allocate(index2(size(x)))

            call sortrx(size(x),x,index2)
